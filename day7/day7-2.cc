...
public:
  ...
  std::vector<size_t> findDecendantSizesLargerThan(size_t min) {
    ...
    for (const auto &[name, dirOrFile] : children) {
      ...
      auto dirSize = dir->calculateTotalSize();
      if (dirSize >= min) {
        result.push_back(dirSize);
      }

      auto subDirs = dir->findDecendantSizesLargerThan(min);
...
int main() {
  ...
  long int updateSize{30000000};
  long int unusedSpace = totalDiskSpace - dir->calculateTotalSize();

  size_t requiredSize = std::abs(unusedSpace - updateSize);

  auto dirSizesLargerThanLimit =
      dir->findDecendantSizesLargerThan(requiredSize);

  // sort dirs by size ascending
  std::sort(std::begin(dirSizesLargerThanLimit),
            std::end(dirSizesLargerThanLimit),
            [](const auto &a, const auto &b) { return a < b; });

  auto sizeOfDirToRemove = dirSizesLargerThanLimit[0];

  std::cout << "Found " << dirSizesLargerThanLimit.size()
            << " dirs larger than limit" << std::endl;

  std::cout << "Need " << requiredSize
            << " bytes to update. Found a dir with size " << sizeOfDirToRemove
            << std::endl;
