...
bool isFullyContained(const Range &r1, const Range &r2) {
  return r1.first >= r2.first && r1.second <= r2.second;
}

size_t readAndExecute(const std::string &filename) {
  size_t numberOfFullyContained = 0;
  ...
  while (std::getline(file, line)) {
    ...
    if (isFullyContained(elfPair.first, elfPair.second) ||
        isFullyContained(elfPair.second, elfPair.first)) {
...
int main() {
  auto numberOfFullyContained = readAndExecute("input.txt");

  std::cout << numberOfFullyContained << std::endl;
